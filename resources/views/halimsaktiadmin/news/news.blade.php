...
<div class="content-wrapper" style="background: white;">
  ...
  <section class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-12">
          <a href="{{ route('admnews_add') }}">
            <button type="button" class="btn btn-success" style="border-radius: 2px 0px 0px 9px; background: black;">Add News</button>
          </a>
          <div class="card">
                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>{{ $no }}</td>
                      <td>
                        <img class="img-thumbnail" src="{{ asset('image/news/'. $n->image) }}">
                        <span style="font-size: 0.8rem; color: #62200a"></span>
                      </td>
                      <td><p>{{ $n->news_nama }}</p></td>
                      <td><p>{{ $n->news_nama_eng }}</p></td>
                      <td><p>{!! $n->description !!}</p></td>
                      <td><p>{!! $n->description_eng !!}</p></td>
                      <td><p>{!! $n->description_sub !!}</p></td>
                      <td><p>{!! $n->description_sub_eng !!}</p></td>

                      <td><a href="{{ route('admnews_edit', $n->id_news) }}" type="button" class="btn btn-info btn-sm">Edit</a> | <a href="{{ route('admnews_delete', $n->id_news) }}" type="button" class="btn btn-danger btn-sm">Delete</a></td>
                    </tr>
                    <?php $no++ ?>
                    @endforeach
                  </tbody>
